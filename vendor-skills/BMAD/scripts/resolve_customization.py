...
def main():
    ...
    # Gambit local change (see vendor-skills/BMAD/ATTRIBUTION.md): an explicit
    # override takes priority over directory-walk discovery. Gambit's vendored
    # skills are installed inside a shared package (npm global prefix, a
    # node_modules tree, etc.) whose ancestors commonly contain an unrelated
    # .git (the package's own repo, a version manager like nvm) that
    # find_project_root(skill_dir) would hit first — deterministically, since
    # that walk never depends on cwd — long before reaching the actual
    # per-goal directory Gambit means by {project-root}. Setting this env var
    # to the resolved goal directory bypasses that ambiguity entirely.
    env_root = os.environ.get("BMAD_PROJECT_ROOT")
    if env_root:
