...
Every skill in this repo reads and writes "`GOAL.md`" as its state file. Which
...
2. **Otherwise, the goal named by `~/.gambit/active`** → read
   `~/.gambit/goals/<slug>/GOAL.md`. (`~/.gambit` is `$GAMBIT_HOME` or
...
4. **Otherwise, if no goals exist anywhere** → hand off to `onboard` for
   first-contact intake. This is the only case with no `GOAL.md` to read yet.
