...
- `lastReviewed` (required, `YYYY-MM-DD`): today's date, every time this skill writes `systemsNotes`. Lets `strategy` and `eval` tell a stale environmental read from a current one without scanning `log`.
